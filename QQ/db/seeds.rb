...
Coment.create(content:"XORの真理値表は配布されたプリントでもあったように、0と1の2種類の入力が来たら1を返すというものでした。
これを2進数で考えてみるとわかりやすいと思います。0+0=0だし、1+1=0(桁あふれ)ですが、1+0=1です。これを用いれば2進数の足し算に使うことができると思います。")
Coment.create(content:"再来週の講義で全加算回路をやるので、そのときに改めて理解が進むと思います。
そこでも分からないということがありましたら、またここに質問でしてください。その際に詳しく説明したいと思います。")


Comment.create(content:"組み合わせ回路と順序回路の違いがいまいちわかりません。どなたかお願いします！＞＜")
Coment.create(content:"組み合わせ回路には時間が関係ありませんが、順序回路には時間が関係してきます。
よって、組み合わせ回路には入力データがそのまま反映されまうが、順序回路はCLKの立ち上がり、立下りによって過去の入力内容がそのまま保持されることもあります")
Coment.create(content:"この説明でだいたい合っています。違いを理解するにはタイミングチャートを書いてみるのがいいと思います。プリントにそれぞれ書いてありますので比較してみてください。順序回路については次の講義で詳しく説明しますので、それでわからないことがあれば、またここに質問してください。")
...
どなたか説明お願いします(T_T)")
Coment.create(content:"本質をいうと、食事などで得たグルコースを材料にして、エネルギー(ATP)を作ることになります。
...
Coment.create(content:"上記の回答で合っています。補足しておくと、グルコースからどうエネルギーが出ているのかというと、分解されたときに不要となったグルコースの結合エネルギーです。クエン酸回路は反応の一連を覚えるのが大切になります。冬休み明けのテストに出ることになっているので、しっかり勉強しておいてください。")
